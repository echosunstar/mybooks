...
\item \colorbox{green!10}{statement} : can interchange $\myint \int$ if $f+$ (in which case can be $\infty$ \textbf{or} if $ \myint \int |f|$ finite where interchange means : $ \myint_{\bs S_1} \big \{ \int_{S_2} f(s_1,s_2)\mu(ds_2) \big \}\ \mu_1(ds_1) \\ = \myint_{\bs S_2} \big \{ \int_{S_1} f(s_1,s_2)\mu(ds_1) \big \}\ \mu_1(ds_2) $
